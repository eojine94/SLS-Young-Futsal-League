...
분류 결과를 브랜치명과 함께 사용자에게 보여주고 확인을 받는다. 반드시 AskUserQuestion 도구를 사용하여 아래 형식으로 표시:
...
사용자가 분류나 브랜치명을 수정하고 싶으면 조정한다.

## Step 3: 그룹별 반복 수행

사용자가 승인하면, **각 그룹별로 아래 3a~3f를 순서대로 반복**한다.
...
- **이상 없음** → `gh pr merge --squash`로 Merge 실행
- **이슈 발견** → 리뷰 이슈 내용을 사용자에게 보고하고 해당 그룹 워크플로우 중단 (PR은 유지, 나머지 그룹은 계속 진행)
...
- 커밋 분류 단계(Step 2)에서만 사용자 확인을 받고, 이후 리뷰 통과 시 Merge까지 자동 수행한다
